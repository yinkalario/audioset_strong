#!/usr/bin/env python3
"""
AudioSet Negative Sample Segmentation Processor

This script processes raw negative metadata to generate fixed-length segmented
negative samples for balanced machine learning training. It converts variable-length
negative sound events into standardized 1-second segments suitable for training
binary classifiers and sound detection models.

Features:
- Segments negative sound events into fixed-length clips (default: 1 second)
- Processes negative samples from segments without target sounds
- Handles duration constraints through intelligent segmentation
- Generates balanced negative samples for all target sound types
- Maintains temporal accuracy for precise negative sample extraction
...
Processing Flow:
1. Load raw negative metadata files for each target sound type
2. Apply segmentation algorithm with duration constraints
3. Generate 1-second negative segment metadata
4. Save segmented negative metadata for model training

Output Structure:
- meta/{target_name}/seg1s/neg_strong/{target_name}_{dataset}_strong.tsv

Usage:
    python generate_seg_neg_strong_meta.py
